...
def guyon_fig6_mat4_redone():

    M = np.array([[0.5, 0.5, 0.5, 0.5],
                 [0.576541, -0.681396, -0.262059, 0.366913],
                 [-0.456946, 0.377955*np.exp(0.63572*1j*np.pi), 0.583663*np.exp(-0.44063*1j*np.pi), 0.554696*np.exp(0.13573*1j*np.pi)],
                 [-0.456946, 0.377955*np.exp(-0.63572*1j*np.pi), 0.583663*np.exp(0.44063*1j*np.pi), 0.554696*np.exp(-0.13573*1j*np.pi)]], dtype=complex)
...
Outputs:
    ...
    The transmission map is one of the nulled outputs that is chosen to create the kernel.
"""
def get_nuller_response2(baseline,fov,sz,base_wavelength):
