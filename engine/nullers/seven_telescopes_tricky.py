...
def heptagon(baseline):
    ...
    R1 = 2*baseline #To the radius of the pentagon
    angles1 = np.linspace(0,2*np.pi,4) #Angular positions of the telescopes
    #To cartesian
    xs1 = R1*np.cos(angles1)[:-1]
    ys1 = R1*np.sin(angles1)[:-1]

    xs1 = np.append(xs0,xs1)
    ys1 = np.append(ys0,ys1)

    R2 = np.sqrt(3)*baseline #To the radius of the pentagon
    angles2 = np.linspace(0,2*np.pi,4)+np.pi/6 #Angular positions of the telescopes
    #To cartesian
    xs2 = R2*np.cos(angles2)[:-1]
    ys2 = R2*np.sin(angles2)[:-1]
